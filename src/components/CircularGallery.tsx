...
// Removed CSS import since we added styles globally in src/app/css/style.css
...
class App {
    ...
    onTouchDown(e) {
        ...
        this.clickStartX = this.start; // Store start position for click detection
    }
    onTouchMove(e) {
    ...
    onTouchUp(e) { // Accept event argument
        this.isDown = false;

        // Detect click (minimal movement)
        const x = e.changedTouches ? e.changedTouches[0].clientX : e.clientX;
        const dist = Math.abs(x - this.clickStartX);

        if (dist < 5 && this.onClick && this.medias && this.medias[0]) {
            // It was a click! Find the active item.
            const width = this.medias[0].width;
            const itemIndex = Math.round(Math.abs(this.scroll.target) / width);
            // Calculate real index in the original items array
            // The gallery duplicates items, so we check specific logic or just use modulo
            const realIndex = itemIndex % this.items.length;

            if (this.items[realIndex]) {
                this.onClick(this.items[realIndex]);
            }
        }
